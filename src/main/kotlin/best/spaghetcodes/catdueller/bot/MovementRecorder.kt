...
object MovementRecorder {
    ...
    /**
     * Start playing a specific pattern by name
     */
    fun startPlayback(patternName: String): Boolean {
        val pattern = patterns.find { it.name.equals(patternName, ignoreCase = true) }
        if (pattern == null) {
            ChatUtils.info("Pattern not found: $patternName")
            return false
        }
        
        return startPlayback(pattern)
    }
     ...
     */
    fun isAutoRecordEnabled() = autoRecordEnabled
    ...
    /**
     * Get current pattern name if playing
     */
    fun getCurrentPatternName() = currentPattern?.name
    ...
    /**
     * Enable auto movement when game is full
     */
    fun enableAutoMovementOnGameFull() {
        autoMovementOnGameFull = true
        ChatUtils.info("Auto movement on game full enabled")
    }
    
    /**
     * Disable auto movement when game is full
     */
    fun disableAutoMovementOnGameFull() {
        autoMovementOnGameFull = false
        ChatUtils.info("Auto movement on game full disabled")
    }
    
    /**
     * Check if auto movement on game full is enabled
     */
    fun isAutoMovementOnGameFullEnabled() = autoMovementOnGameFull
