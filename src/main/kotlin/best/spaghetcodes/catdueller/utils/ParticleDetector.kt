...
object ParticleDetector {
                ...
                for (field in packetClass.declaredFields) {
                    ...
                    when {
                        ...
                        value is Float || value is Double -> {
                            ...
                            when (coordCount) {
                                0 -> { x = doubleValue; coordCount++ }
                                1 -> { y = doubleValue; coordCount++ }
                                2 -> { z = doubleValue; coordCount++ }
                            }
        ...
        for ((particle, _) in recentParticles) {
            if (particle.type == particleType) {
                ...
                if (distance <= radius) {
                    ...
                    found = true
                    if (debug) {
                        println("[ParticleDetector] Found ${particleType.particleName} at distance ${"%.2f".format(distance)}")
                    }
                }
            }
        }
     ...
     */
    fun hasPortalParticleNearby(x: Double, y: Double, z: Double, radius: Double = 3.0, debug: Boolean = false): Boolean {
     ...
     */
    fun hasRedstoneParticleNearby(x: Double, y: Double, z: Double, radius: Double = 3.0, debug: Boolean = false): Boolean {
